    ...
    <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 backdrop-blur-xl shadow-2xl text-slate-900">
      ...
      {!showResult ? (
        <div className="space-y-4">
          {QUESTIONS.map((q, idx) => (
            <div key={q.id} className="p-4 bg-slate-100/40 rounded-2xl border border-slate-200 hover:border-slate-200 transition">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 self-end md:self-center shrink-0">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-sm font-semibold transition flex items-center gap-1.5 cursor-pointer ${
                      answers[q.id] === false
                        ? 'bg-rose-600 text-white shadow-lg shadow-xs'
                        : 'bg-slate-100 text-slate-600 hover:bg-slate-700'
                    }`}
                  >
        ...
        <div className="space-y-6">
          <div className="p-6 md:p-8 bg-gradient-to-br from-slate-900 to-indigo-950/60 rounded-3xl border border-teal-200 text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 text-left">
              {QUESTIONS.map((q) => (
                <div key={q.id} className="p-3.5 bg-white rounded-xl border border-slate-200 flex items-start gap-2.5">
                  {answers[q.id] ? (
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
              <button
                onClick={resetAudit}
                className="px-6 py-3 bg-slate-100 hover:bg-slate-700 text-slate-600 rounded-xl font-semibold flex items-center gap-2 transition text-sm cursor-pointer"
              >
