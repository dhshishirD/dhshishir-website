  ...
  return (
    <section id="experience" className="py-20 relative bg-white border-t border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-16 bg-gradient-to-r from-slate-50 via-indigo-50/50 to-slate-50 border border-indigo-200 rounded-3xl p-6 sm:p-8 shadow-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {MEDIA_MENTIONS.map((media) => (
              <a
                ...
                className="p-4 bg-white rounded-2xl border border-slate-200 hover:border-blue-400 hover:shadow-md transition flex flex-col justify-between group"
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-[11px]">
                    <span className="font-bold text-emerald-700">{media.outlet}</span>
                    <span className="text-[10px] bg-slate-100 text-slate-700 font-semibold px-2 py-0.5 rounded-full">{media.dateBadge}</span>
                  </div>
                  <p className="text-xs font-semibold text-slate-800 line-clamp-2 group-hover:text-blue-900 transition">
                    {media.title}
                  </p>
                </div>
                <div className="pt-3 flex items-center gap-1 text-[11px] font-bold text-blue-700 group-hover:text-blue-900">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="space-y-6">
              {EXPERIENCES.map((item) => (
                <div
                  key={item.id}
                  className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-7 hover:border-slate-300 shadow-xs transition"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
                    <div>
                      <h4 className="text-base font-bold text-slate-900">{item.role}</h4>
                      <div className="text-sm font-semibold text-emerald-700">{item.organization}</div>
                    </div>
                    {item.badge && (
                      <span className="self-start sm:self-center text-[11px] font-bold bg-blue-50 text-blue-900 border border-blue-200 px-3 py-1 rounded-full">
                        {item.badge}
                      </span>
                    )}
                  </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="space-y-4">
              ...
              {EDUCATION_DATA.map((edu, idx) => (
                <div key={idx} className="p-5 bg-white rounded-2xl border border-slate-200 shadow-xs space-y-2">
                  ...
                  <div className="text-xs font-bold text-blue-800">{edu.institution}</div>
            ...
            <div className="space-y-4">
              ...
              {CERTIFICATIONS.map((cert, idx) => (
                <div key={idx} className="p-5 bg-white rounded-2xl border border-slate-200 shadow-xs space-y-1.5">
                  <h4 className="text-xs font-bold text-slate-900">{cert.title}</h4>
                  <div className="text-xs text-amber-800 font-bold">{cert.issuer}</div>
