...
export const DiagnosticResultCard: React.FC<DiagnosticResultCardProps> = ({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 backdrop-blur-xl shadow-2xl text-slate-900 space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-6 border-b border-slate-200">
        <div className="space-y-2 text-center md:text-left">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-teal-50 text-teal-900 text-xs font-bold border border-teal-200">
            <Sparkles className="w-3.5 h-3.5 text-teal-800" /> Official Diagnostic Report Card
          </div>
          <h3 className="text-2xl sm:text-3xl font-black text-slate-900">
            Placement Level: <span className="text-teal-800">{result.cefrLevel} — {result.levelTitle}</span>
          </h3>
        ...
        {result.flaggedWeakPatterns.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {result.flaggedWeakPatterns.map((key) => {
              ...
              if (!info) return null;
              return (
                <div key={key} className="p-4 bg-white rounded-2xl border border-amber-500/30 space-y-2">
                  <div className="flex items-center justify-between text-xs font-bold text-amber-400">
                    <span>{info.name}</span>
                    <span className="text-[10px] px-2 py-0.5 rounded bg-teal-50 text-amber-300 font-medium">Action Needed</span>
                  </div>
      ...
      <div className="pt-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
        <button
          onClick={onRetake}
          className="w-full sm:w-auto px-5 py-3 rounded-xl bg-slate-100 hover:bg-slate-700 text-slate-800 text-xs font-bold flex items-center justify-center gap-2 transition cursor-pointer"
        >
