        ...
        <div className="p-6 bg-white rounded-2xl border border-slate-200 flex flex-col items-center justify-center text-center space-y-4">
          {isListening ? (
            <div className="space-y-3">
              <div className="w-14 h-14 rounded-full bg-rose-50 border border-rose-200 text-rose-800 font-bold flex items-center justify-center mx-auto animate-pulse">
                <Mic className="w-7 h-7" />
              </div>
              <div className="text-sm font-bold text-rose-800 font-bold font-mono animate-pulse">
                LISTENING TO YOUR SPEECH...
              </div>
              <p className="text-xs text-slate-600">Speak the target sentence clearly into your microphone.</p>
              <button
                onClick={stopListening}
                className="px-5 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-900 font-semibold rounded-xl text-xs font-bold flex items-center gap-2 mx-auto cursor-pointer"
              >
            ...
            <div className="space-y-3">
              <button
                ...
                className="px-6 py-3.5 bg-white border border-slate-200 hover:bg-teal-800 disabled:opacity-50 text-slate-900 rounded-2xl font-bold text-sm flex items-center gap-2 shadow-lg shadow-xs transition transform active:scale-95 cursor-pointer mx-auto"
              >
          ...
          {recognitionError && (
            <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl text-rose-900 font-semibold text-xs flex items-start gap-2 max-w-md text-left">
              <AlertTriangle className="w-4 h-4 text-rose-800 font-bold shrink-0 mt-0.5" />
        ...
        {alignmentResult && (
          <div className="space-y-6 pt-4 border-t border-slate-200">
            ...
            <div className="p-5 bg-white rounded-2xl border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                ...
                <div className="text-2xl sm:text-3xl font-black text-slate-900 flex items-center gap-2">
                  <span className={matchScore && matchScore >= 80 ? 'text-teal-800' : matchScore && matchScore >= 50 ? 'text-amber-400' : 'text-rose-800 font-bold'}>
                    {matchScore}%
                  </span>
            ...
            <div className="flex items-center justify-end pt-3">
              <button
                onClick={handleNext}
                className="px-5 py-2.5 bg-teal-900 hover:bg-teal-800 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition cursor-pointer"
              >
