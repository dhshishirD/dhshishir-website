      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredStudioWords.map(item => {
          ...
          return (
            <div
              key={item.id}
              className="bg-white border border-slate-200 hover:border-teal-200 rounded-3xl p-5 transition space-y-4 shadow-xl flex flex-col justify-between group"
            >
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {isShift && (
                    <button
                      ...
                      className="text-[11px] font-bold px-2.5 py-1 rounded-lg bg-teal-50 text-teal-900 border border-teal-200 hover:bg-teal-50 flex items-center gap-1.5 transition cursor-pointer"
                    >
                      <ArrowRightLeft className="w-3 h-3 text-teal-800" />
                ...
                {/* Word Title & Meaning */}
                <div>
                  <h4 className="text-xl font-black text-slate-900 capitalize flex items-center gap-2">
                    <span>{item.word}</span>
                    {isShift && (
                      <span className={`text-xs px-2 py-0.5 rounded font-bold uppercase ${
                        isVerb ? 'bg-teal-50 text-amber-300 border border-amber-500/30' : 'bg-teal-50 text-teal-900 border border-teal-200'
                      }`}>
                        {isVerb ? 'Verb' : 'Noun'}
                      </span>
                    )}
