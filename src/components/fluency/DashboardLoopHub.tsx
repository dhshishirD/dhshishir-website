...
export const DashboardLoopHub: React.FC<DashboardLoopHubProps> = ({ onNavigateStage }) => {
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950/40 to-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="space-y-2 text-center md:text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 text-xs font-bold uppercase tracking-wider">
            <Compass className="w-3.5 h-3.5 text-indigo-400" /> Stage 6 • Continuous Learning Loop
          </div>
      ...
      <div className="p-6 bg-slate-950 rounded-3xl border border-slate-800 space-y-4 shadow-xl">
        <div className="flex items-center justify-between">
          <div className="text-xs font-bold text-white uppercase tracking-wider flex items-center gap-2">
            <RotateCcw className="w-4 h-4 text-indigo-400" />
            Interactive Looped Learning Cycle
          </div>
          <span className="text-[11px] text-slate-400 font-medium">Click any node to navigate</span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
          ...
          <button
            ...
            className="p-3.5 bg-slate-900/90 hover:bg-slate-800/90 rounded-2xl border border-emerald-500/40 text-left space-y-1 transition cursor-pointer"
          >
            <div className="text-[10px] font-bold text-emerald-400">STAGE 1</div>
            <div className="text-xs font-bold text-white flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
              <span>Diagnose</span>
            </div>
            <div className="text-[10px] text-slate-400">Placement Quiz</div>
          </button>

          <button
            onClick={() => onNavigateStage('stage2')}
            className="p-3.5 bg-slate-900/90 hover:bg-slate-800/90 rounded-2xl border border-indigo-500/40 text-left space-y-1 transition cursor-pointer"
          >
            <div className="text-[10px] font-bold text-indigo-400">STAGE 2</div>
            <div className="text-xs font-bold text-white flex items-center gap-1">
              <BookOpen className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
              <span>Learn & Drill</span>
            </div>
            <div className="text-[10px] text-slate-400">IPA & Minimal Pairs</div>
          </button>

          <button
            onClick={() => onNavigateStage('stage3')}
            className="p-3.5 bg-slate-900/90 hover:bg-slate-800/90 rounded-2xl border border-purple-500/40 text-left space-y-1 transition cursor-pointer"
          >
            ...
            <div className="text-[10px] text-slate-400">Audio Comparison</div>
          </button>

          <button
            onClick={() => onNavigateStage('stage4')}
            className="p-3.5 bg-slate-900/90 hover:bg-slate-800/90 rounded-2xl border border-amber-500/40 text-left space-y-1 transition cursor-pointer"
          >
            ...
            <div className="text-[10px] text-slate-400">Prompts & IELTS</div>
          </button>

          <button
            onClick={() => onNavigateStage('stage5')}
            className="p-3.5 bg-slate-900/90 hover:bg-slate-800/90 rounded-2xl border border-cyan-500/40 text-left space-y-1 transition cursor-pointer"
          >
            <div className="text-[10px] font-bold text-cyan-400">STAGE 5</div>
            <div className="text-xs font-bold text-white flex items-center gap-1">
              <Activity className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
              <span>Get Feedback</span>
            </div>
            <div className="text-[10px] text-slate-400">Speech Engine</div>
          </button>
          ...
          <div className="p-3.5 bg-gradient-to-br from-indigo-950/80 to-slate-900 rounded-2xl border border-indigo-400 ring-1 ring-indigo-400/40 text-left space-y-1 shadow-lg">
            <div className="text-[10px] font-bold text-indigo-300">STAGE 6</div>
            <div className="text-xs font-bold text-white flex items-center gap-1">
              <RotateCcw className="w-3.5 h-3.5 text-indigo-300 shrink-0" />
              <span>Repeat Loop</span>
            </div>
            <div className="text-[10px] text-indigo-300 font-medium">Active Dashboard</div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        ...
        <div className="p-6 bg-slate-900/90 rounded-3xl border border-slate-800 space-y-2">
          <div className="text-xs text-slate-400 font-medium">Verified CEFR Level</div>
        ...
        <div className="p-6 bg-slate-900/90 rounded-3xl border border-slate-800 space-y-2">
          <div className="text-xs text-slate-400 font-medium">Accuracy Benchmark</div>
        ...
        <div className="p-6 bg-slate-900/90 rounded-3xl border border-slate-800 space-y-2">
          ...
          <div className="text-3xl font-black text-amber-400 flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 font-normal">sound patterns</span>
          </div>
          <div className="text-xs text-slate-500">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400">Auto-routed from your diagnosis</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {weakList.length > 0 ? (
            weakList.map((key) => {
              ...
              if (!info) return null;
              return (
                <div key={key} className="p-5 bg-slate-900/90 rounded-2xl border border-amber-500/30 space-y-3 shadow-lg flex flex-col justify-between">
                  <div className="space-y-2">
                    ...
                    <div className="text-[11px] text-slate-400 bg-slate-950 p-2.5 rounded-xl border border-slate-800 font-mono">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-800">
                    ...
                    <button
                      onClick={() => onNavigateStage('stage3')}
                      className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold flex items-center gap-1 transition cursor-pointer"
                    >
      ...
      <div className="p-6 bg-slate-950 rounded-3xl border border-slate-800 space-y-5 shadow-xl">
      ...
      <div className="p-6 bg-gradient-to-r from-indigo-950/60 via-slate-900 to-indigo-950/60 rounded-3xl border border-indigo-500/30 flex flex-col sm:flex-row items-center justify-between gap-5 shadow-2xl">
        ...
        <button
          onClick={() => onNavigateStage('stage1')}
          className="px-6 py-3 bg-gradient-to-r from-emerald-600 to-indigo-600 hover:from-emerald-500 hover:to-indigo-500 text-white rounded-2xl text-xs font-bold flex items-center gap-2 shadow-lg shadow-emerald-900/40 transition transform active:scale-95 cursor-pointer whitespace-nowrap"
        >
